...
inclination = rad2deg(acos(angularMomentumVector(3)/angularMomentumMagnitude));
...
RAAN = rad2deg(acos(NVector(1)/NMagnitude));
if NVector(2) < 0
    RAAN = 360 - RAAN;
end

% Finding argument of perigee, omega
argumentOfPerigee = rad2deg(acos((dot(NVector, eccentricityVector))/(NMagnitude*eccentricityMagnitude)));
if eccentricityVector(3) < 0
    argumentOfPerigee = 360 - argumentOfPerigee;
end

% Finding true anomaly, theta
trueAnomaly = rad2deg(acos((dot(eccentricityVector, positionVector))/(eccentricityMagnitude * positionMagnitude)));
